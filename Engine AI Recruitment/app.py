...
from modules.thread_module.thread_handler import run_in_thread

db_ops = DatabaseOperations()
file_handler = FileHandler()
gpt_engine = GPTEngine()

cors_config_path = 'modules/cors_module/cors_config.json'
with open(cors_config_path, 'r') as cors_config_file:
    cors_config = json.load(cors_config_file)
    cors_origins = list(cors_config.values())  # Mengambil semua nilai dari cors_config
...
app.config['SESSION_COOKIE_DOMAIN'] = '.linovhr.com'  # Jika menggunakan subdomain, awali dengan titik
app.config['SESSION_COOKIE_PATH'] = '/'  # Untuk memastikan cookie tersedia di seluruh aplikasi
...
# CORS Configuration
cors_origins = ["https://api-dev-airecruitment.linovhr.com", "http://localhost:4200", "https://dev5.linovhr.com", "https://hcms.linovhr.com", "https://dev-jobportal2.linovhr.com", "https://jobportal2.linovhr.com", "https://*.linovhr.com", "http://172.31.48.31:54321", "http://localhost:*"]
cors = CORS(app, supports_credentials=True, resources={r"/*": {"origins": cors_origins}})

@app.after_request
def after_request(response):
    response.headers.add('Access-Control-Allow-Headers', 'Content-Type,Authorization,Your-Custom-Header')
    response.headers.add('Access-Control-Allow-Methods', 'GET,POST,PUT,DELETE,OPTIONS,PATCH')
    response.headers.add('Access-Control-Allow-Credentials', 'true')  # Jika Anda ingin mengizinkan cookies atau sesi lintas domain
    return response
...
def initialize_api_type_thread(session_data, api_type):
    if not isinstance(session_data, dict):
        raise TypeError("session_data should be a dictionary")

    current_time = datetime.now().strftime("%H-%M-%S")
    session_id = str(uuid.uuid4())  # Generate a unique session ID if needed
    session_data['gpt_api_type'] = f"{api_type}_{session_id}_{current_time}"
...
def query_gpt_thread_safe(session_data={}, primary_key={}, additional_dict={}, one_time_message=''):
    # Ensure 'history' and 'usage' exist in session data
    session_data.setdefault('history', [{"role": "system", "content": "Initializing conversation with GPT."}])
    session_data.setdefault('usage', [])
    session_data.setdefault('bool_chat', False)  # Add default value for 'bool_chat'

    # Query GPT
    answer, json_response = gpt_engine.hit_groq_api(session_data, one_time_message)
    # answer, json_response = gpt_engine.hit_gpt_api(session, one_time_message)
    session_data['history'].append({"role": "assistant", "content": answer})
    usage = json_response.get('usage', {})
    session_data['usage'].append(str(usage))

    # Log and format the response
    log_file_content = file_handler.save_chat_transcript(session_data)
    dict_form, answer = gpt_engine.format_response_gpt(session_data, answer)
    dict_form = {**additional_dict, **dict_form}

    # Ensure 'engine_type' is valid in db_ops session_types
    engine_type = session_data.get('gpt_api_type', 'default_type').split('_')[0]
    if engine_type not in db_ops.session_types:
        engine_type = 'fallback_type'

    # Store to DB
    try:
        row_id = db_ops.store_to_db(
            engine_type,
        ...
        logging.error(f"Error in store_to_db for engine_type '{engine_type}': {e}")
        raise e
...
# Decorator for routes requiring a valid JWT token
def token_required(f):
    def decorated(*args, **kwargs):
        ...
        try:
            decoded_claims = decode_without_verification(token)
            if not decoded_claims:
                return jsonify({'message': 'Invalid token!'}), 401

            # Manually map the claims to tenant, source, and company based on known encoded values
...
class create_cv_extractor(Resource):
    parser = create_parser(True, candidate_id={'type': str, 'required': True, 'help': 'Candidate ID', 'location': 'form'})  # Accept candidate_id
    ...
    def post(self):
        try:
            ...
            candidate_id = args['candidate_id']  # Get candidate ID
            
            # Generate a unique id_cv for this CV extraction
...
class create_cvs_extractor_multiple(Resource):
    ...
    def post(self):
        try:
            start_time = time.time()  # Start the total processing time
            ...
            # Parse candidate_ids directly from form and remove extra spaces
            candidate_ids = request.form.get('candidate_ids', '').split(',')
            candidate_ids = [id.strip() for id in candidate_ids]  # Removing any unwanted spaces
            print("Candidate IDs: ", candidate_ids)
            
            # Validate the number of files and candidate IDs
            ...
            for idx, uploaded_file in enumerate(uploaded_files):
                # Track the start time for processing each CV
                ...
                # Log how long it took to process each CV
                cv_end_time = time.time()
                cv_processing_time = cv_end_time - cv_start_time
                print(f"Time taken to process CV for candidate {candidate_id}: {cv_processing_time:.2f} seconds")

            # Log total processing time
            total_end_time = time.time()
            total_processing_time = total_end_time - start_time
            print(f"Total processing time for all CVs: {total_processing_time:.2f} seconds")
...
# Atur konfigurasi logging untuk menampilkan waktu dan level logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

@ns.route('/cv_extractor_multiple_thread')  # POST
class create_cvs_extractor_multiple_thread(Resource):
    parser = reqparse.RequestParser()
    parser.add_argument('cvs_file', type=FileStorage, location='files', action='append', required=True, help='CV files')  # Multiple files allowed
    ...
    def post(self):
        try:
            ...
            # Define processing function with `session_data` to avoid Flask context issues
            def process_single_cv(uploaded_file, candidate_id, session_data, responses, thread_idx):
                try:
                    thread_start_time = time.time()  # Record the start time for this thread
                    print(f"Thread {thread_idx} - Processing file {uploaded_file.filename} for candidate ID {candidate_id}")
                    ...
                    # Use `initialize_api_type_thread` to initialize api_type in thread-safe way
                    initialize_api_type_thread(session_data, 'Form Filler')
                    ...
                    # Add the starting prompt to the session history
                    session_data['history'] = [{"role": "user", "content": starting_prompt}]
                    print(f"Starting prompt prepared for candidate {candidate_id}")

                    # Interact with the GPT engine and generate the response
                    dict_form, answer = query_gpt_thread_safe(
                    ...
                    # Log thread completion time
                    thread_end_time = time.time()  # Record the end time for this thread
                    thread_duration = thread_end_time - thread_start_time  # Calculate the duration of the thread
                    print(f"Thread {thread_idx} completed in {thread_duration:.2f} seconds.")
                except Exception as e:
                    print(f"Error processing candidate {candidate_id}: ", e)
                    traceback.print_exc()
                    responses.append({'candidate_id': candidate_id, 'message': str(e), 'status': 500})

            # Copy session data as a dictionary to use within each thread
            session_data = session.copy()
            
            # Start threads for each file and candidate ID
            for idx, uploaded_file in enumerate(uploaded_files):
                candidate_id = candidate_ids[idx]
                thread = Thread(target=process_single_cv, args=(uploaded_file, candidate_id, session_data, responses, idx + 1))
            ...
            # Calculate the total time for all threads to complete
            end_time = time.time()
            total_duration = (end_time - start_time) / 60  # Convert to minutes
            print(f"All threads completed processing in {total_duration:.2f} minutes.")
...
@ns.route('/candidate_redflag_thread')  # POST
class create_candidate_redflag_thread(Resource):
    parser = reqparse.RequestParser()
    parser.add_argument('candidate_ids', type=str, location='form', required=True, help='List of Candidate IDs separated by commas')
    parser.add_argument('job_desc', type=str, location='form', required=True, help='Job Description')
    ...
    def post(self):
        try:
            ...
            args = self.parser.parse_args()
            candidate_ids = args['candidate_ids'].split(',')  # Split the candidate IDs by commas
            job_description = args['job_desc']
            ...
            # Define a function to process each candidate's redflag check
            def process_candidate_redflag(candidate_id, session_data, responses, thread_idx):
                try:
                    thread_start_time = time.time()  # Record the start time for this thread
                    print(f"Thread {thread_idx} - Processing Candidate ID {candidate_id}")

                    # Fetch the CV content for the candidate
                    candidate_data = db_ops.get_row_data({'candidate_id': candidate_id.strip()}, 'cv_directories', all_row=False)
                    if not candidate_data:
                        responses.append({'candidate_id': candidate_id, 'message': 'Candidate CV not found', 'status': 404})
                        return

                    str_cv = candidate_data['str_content']
                    id_cv = candidate_data['id_cv']

                    # Initialize GPT session for Candidate Redflags
                    initialize_api_type_thread(session_data, 'Candidate Redflags')

                    # Build the starting prompt for GPT
                    starting_prompt = gpt_engine.initialize_candidate_redflags()
                    starting_prompt += '\n------\nJob Vacancy:\n' + session_data['job_vacancy']
                    starting_prompt += '\n------\nCV:\n' + str_cv
                    starting_prompt += f'\nTenant: {tenant}\nSource: {source}\nCompany: {company}'

                    session_data['history'] = [{"role": "user", "content": starting_prompt}]

                    # Query GPT and format the result
                    dict_form, answer = query_gpt_thread_safe(
                        session_data,
                        primary_key={"candidate_id": candidate_id.strip(), "id_cv": id_cv},
                        additional_dict={"candidate_id": candidate_id.strip(), "id_cv": id_cv}
                    )
                    print(f"GPT response received for candidate {candidate_id}")
                    ...
                    # Append the processed data to responses
                    responses.append({"candidate_id": candidate_id, **dict_form, 'status': 200})
            ...
            # Start threads for each candidate
            for idx, candidate_id in enumerate(candidate_ids):
                thread = Thread(target=process_candidate_redflag, args=(candidate_id, session_data, responses, idx + 1))
                threads.append(thread)
                thread.start()
            ...
            total_end_time = time.time()
            total_processing_time = total_end_time - start_time
            print(f"Total processing time for all candidates: {total_processing_time:.2f} seconds")
        ...
        try:
            ...
            for candidate_id in candidate_ids:
                try:
                    ...
                    logging.error(f"Error fetching CV for candidate id {candidate_id}: {str(e)}")
            ...
            logging.error(f"Error in create_candidate_ranker: {str(e)}")
            return {'message': str(e)}, 400
    ...
    def extract_score(item):
        ...
        if 'skor' in item and isinstance(item['skor'], str):
            try:
                ...
                logging.error(f"Failed to parse score from {item['skor']}")
                return 0
        else:
            # Log missing 'skor' field and return a default score of 0
            logging.error(f"'skor' field missing in item: {item}")
            return 0
...
@ns.route('/candidate_ranker_thread')  # POST
class create_candidate_ranker_thread(Resource):
    parser = reqparse.RequestParser()
    parser.add_argument('candidate_ids', type=str, location='form', required=True, help='List of Candidate IDs separated by commas')
    parser.add_argument('job_requirements', type=str, location='form', required=True, help='Job Requirements')
    parser.add_argument('job_title', type=str, location='form', required=True, help='Job Title')
    parser.add_argument('job_description', type=str, location='form', required=True, help='Job Description')
    ...
    def post(self):
        try:
            ...
            # Retrieve tenant, source, and company from session
            tenant = session.get('tenantCode')
            source = session.get('source')
            company = session.get('company')
            ...
            # Parse input arguments
            args = self.parser.parse_args()
            candidate_ids = args['candidate_ids'].split(',')  # Split the candidate IDs by comma
            job_requirements = args['job_requirements']
            job_title = args['job_title']
            job_description = args['job_description']

            # Initialize the session for Candidate Ranker
            session['job_vacancy'] = job_requirements
            session['job_title'] = job_title
            session['job_description'] = job_description
            session['usage'], session['bool_chat'] = [], False
            ...
            # Define a function to process the candidate ranking
            def process_single_candidate_ranker(candidate_id, session_data, responses, thread_idx):
                try:
                    thread_start_time = time.time()  # Record the start time for this thread
                    ...
                    # Initialize GPT session for Candidate Ranker
                    initialize_api_type_thread(session_data, 'Candidate Ranker')
                    ...
                    # Query GPT and format the result
                    dict_form, answer = query_gpt_thread_safe(session_data, primary_key={"candidate_id": candidate_id.strip()}, additional_dict={"candidate_id": candidate_id.strip()})
                    print(f"GPT response received for candidate {candidate_id}")

                    # Append the processed form data to responses
                    responses.append({"candidate_id": candidate_id, **dict_form, 'status': 200})
            ...
            # Start threads for each candidate
            for idx, candidate_id in enumerate(candidate_ids):
                thread = Thread(target=process_single_candidate_ranker, args=(candidate_id, session_data, responses, idx + 1))
                threads.append(thread)
                thread.start()
...
@ns.route('/candidate_profile_matching_thread')  # POST
class create_candidate_profile_matching_thread(Resource):
    parser = reqparse.RequestParser()
    parser.add_argument('candidate_ids', type=str, location='form', required=True, help='List of Candidate IDs separated by commas')
    parser.add_argument('job_desc', type=str, location='form', required=True, help='Job Description')
    parser.add_argument('kriteria_pekerjaan', type=str, location='form', required=True, help='Job Criteria')
    #kriteria_pekerjaan = job_requirements
    ...
    def post(self):
        try:
            ...
            candidate_ids = args['candidate_ids'].split(',')  # Split the candidate IDs by comma
            job_description = args['job_desc']
            kriteria_pekerjaan = args['kriteria_pekerjaan'].split('. ')  # Split job criteria into a list
            ...
            # Define a function to process each candidate profile matching
            def process_single_candidate_profile_matching(candidate_id, session_data, responses, thread_idx):
                try:
                    thread_start_time = time.time()  # Record the start time for this thread
                    ...
                    candidate_data = db_ops.get_row_data({'candidate_id': candidate_id.strip()}, 'cv_directories', all_row=False)
                    str_cv = candidate_data['str_content']
                    id_cv = candidate_data['id_cv']  # Retrieve id_cv for the candidate
                    
                    # Initialize GPT session for Candidate Profile Matching
                    initialize_api_type_thread(session_data, 'Candidate Profile Matching')

                    # Build the starting prompt for GPT
                    starting_prompt = gpt_engine.initialize_candidate_profile_matching()
                    starting_prompt += '\n------\n' + session_data['job_vacancy']
                    starting_prompt += '\n------\nJob Criteria:\n' + '\n'.join(session_data['kriteria_pekerjaan'])
                    starting_prompt += '\n------\nCV:\n' + str_cv
                    starting_prompt += f'\nTenant: {tenant}\nSource: {source}\nCompany: {company}'
            ...
            # Start threads for each candidate
            for idx, candidate_id in enumerate(candidate_ids):
                thread = Thread(target=process_single_candidate_profile_matching, args=(candidate_id, session_data, responses, idx + 1))
                threads.append(thread)
                thread.start()
...
class InitializeInterview(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            logging.info(f"Interview dimulai dengan room_id {room_id}. Timer {interview_duration_minutes} menit diaktifkan.")
            Timer(interview_duration_seconds, finish_interview, args=[room_id]).start()

            return jsonify({"room_id": room_id, "message": result})

        except Exception as e:
            logging.error(f"Error during interview initialization: {str(e)}", exc_info=True)
            return {'message': str(e)}, 400
...
class AnsweringInterview(Resource):
    ...
    def post(self):
        try:
            ...
            # Logging input data
            logging.info(f"Received room_id: {room_id} and user_input: {user_input}")

            # Load session state
            session_state = db_ops.get_session_state(room_id)

            if not session_state:
                logging.error(f"Room with room_id: {room_id} not found")
            ...
            if session_state.get('interview_finished', False):
                logging.info(f"Interview telah selesai untuk room_id: {room_id}")
            ...
            try:
                ...
                logging.error(f"Invalid room_id format: {room_id}")
            ...
            # Logging result from GPT
            logging.info(f"AI response for room_id {room_id}: {result}")
            ...
            logging.error(f"Error in answering interview: {str(e)}", exc_info=True)
            return jsonify({'message': f'Something went wrong: {str(e)}'}), 500
