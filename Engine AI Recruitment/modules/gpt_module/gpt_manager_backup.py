...
class GPTEngine:
    ...
    def format_candidate_redflags(self, answer):
        ...
        for i in result:
            if not i:  # Skip empty strings
                continue

            # Parse the name
            if 'name' in i.lower() and ':' in i:
                name_line = i.split(':', 1)
                dict_form['name'] = name_line[1].strip()
                continue

            # Split other fields by the first colon
            if ':' in i:
                key, value = i.split(':', 1)
                key = key.strip('\n').strip()
                value = value.strip('\n').strip()

                # If the status is 'Safe', remove 'Safe <->' from the description
                if 'safe' in value.lower():
                    all_status_counter += 1
                    # Remove 'Safe <->' from the description if it's present
                    desc = value.replace('Safe <->', '').strip()
                    dict_form[key] = {"status": "Safe", "desc": desc}
                else:
                    all_status_counter += 1
                    redflag_counter += 1
                    if '<->' in value:
                        ...
                        status, desc = 'Red Flag', value.strip()
                    dict_form[key] = {"status": status.strip('\n').strip(), "desc": desc.strip('\n').strip()}

        # Calculate redflag percentage
    ...
    def candidate_ranker_formatting(self, llm_response):
        result = {
            'ranking': self.candidate_ranker_parse(llm_response, 'Ranking', '\n') or 'Tidak tersedia',
            'name': self.candidate_ranker_parse(llm_response, 'Name :', '\n') or 'Tidak tersedia',
            'total_masa_kerja_relevan': self.candidate_ranker_parse(llm_response, 'Total Masa Kerja Relevan :', '\n') or 'Tidak tersedia',
            'skills': self.candidate_ranker_parse(llm_response, 'Skills :', '\n') or 'Tidak tersedia',
            'kelebihan': self.candidate_ranker_parse(llm_response, 'Kelebihan :', '\n') or 'Tidak tersedia',
            'kekurangan': self.candidate_ranker_parse(llm_response, 'Kekurangan :', '\n') or 'Tidak tersedia',
            'skor': self.candidate_ranker_parse(llm_response, 'Skor :', '\n') or 'Tidak tersedia',
            'alasan': self.candidate_ranker_parse(llm_response, 'Alasan :', '\n') or 'Tidak tersedia',
            'kesimpulan': self.candidate_ranker_parse(llm_response, 'Kesimpulan :', '\n') or 'Tidak tersedia',
            'job_title': self.candidate_ranker_parse(llm_response, 'Job Title :', '\n') or 'Tidak tersedia',
            'job_requirements': self.candidate_ranker_parse(llm_response, 'Job Requirements :', '\n') or 'Tidak tersedia',
            'responsibilities': self.candidate_ranker_parse(llm_response, 'Responsibilities :', '\n') or 'Tidak tersedia',
        }
        return result
...
# ---------------------------------------------- Interview ---------------------------------------------------
    ...
    def initialize_interview(self, str_cv='', str_job_req='', interview_duration_minutes=60):
        ...
        # Replace Job Requirement
        if str_job_req == '':
            prompt = prompt.replace("<-> Job Requirement <->", job_req)
        else:
            prompt = prompt.replace("<-> Job Requirement <->", str_job_req)

        # Replace interview duration
        duration_str = self.interview_duration(interview_duration_minutes)
        prompt = prompt.replace('<-> waktu interview <->', duration_str)

        return prompt
    
    def interview_duration(self, duration_minutes=60):
        """
        Returns a formatted string of interview duration.
        Default is 60 minutes, but it can be customized.
        """
